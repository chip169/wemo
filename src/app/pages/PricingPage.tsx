...
const plans = [
  {
    ...
    price: "199.000đ",
    period: "/quà",
    description: "Hoàn hảo để khám phá WEBO lần đầu",
    color: "#FFD4D4",
    features: [
...
const faqs = [
  { q: "Tôi có thể nâng cấp gói sau này không?", a: "Có! Bạn có thể nâng cấp từ Cơ Bản lên Cao Cấp hoặc Doanh Nghiệp bất cứ lúc nào. Sự chênh lệch giá sẽ được tính theo tỷ lệ." },
  { q: "Gói Cơ Bản có thời hạn không?", a: "Không! Một lần thanh toán, trải nghiệm tồn tại mãi mãi. Người nhận có thể xem lại bất cứ lúc nào." },
  { q: "Gói Doanh Nghiệp phù hợp với đám cưới không?", a: "Hoàn toàn phù hợp! Chúng tôi hỗ trợ tạo hàng trăm quà cùng lúc, lý tưởng cho đám cưới, sự kiện công ty và hội nghị." },
];
  ...
  return (
    <div className="pt-20" style={{ background: "#FAF8F5" }}>
      ...
      <section className="py-20 text-center px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p className="max-w-2xl mx-auto" style={{ fontSize: "1.125rem", color: "#6B6B6B", lineHeight: 1.6 }}>
            Không phí ẩn, không bất ngờ. Chọn gói phù hợp và bắt đầu tạo ký ức ngay hôm nay.
          </p>
        </motion.div>
      </section>
            ...
            >
              ...
              <div
                ...
                style={{
                  background: plan.popular
                    ...
                    : "white",
                  border: plan.popular ? "2px solid #E8B4A8" : "1px solid rgba(0,0,0,0.08)",
              ...
              >
                <div
                  ...
                  style={{ background: plan.color, boxShadow: `0 8px 20px ${plan.color}50` }}
                >
                  <plan.icon className="w-7 h-7 text-white" />
                </div>

                <h3 className="mb-1" style={{ fontSize: "1.75rem", fontWeight: 700, color: "#1A1818" }}>
                  {plan.name}
                </h3>
                <p className="mb-6" style={{ color: "#6B6B6B", fontSize: "0.9375rem" }}>
                  {plan.description}
                </p>

                <div className="mb-8 flex items-baseline gap-2">
                  <span style={{ fontSize: "2.75rem", fontWeight: 700, color: "#1A1818" }}>{plan.price}</span>
                ...
                <ul className="space-y-3 mb-8 flex-1">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span style={{ color: "#1A1818", fontSize: "0.9375rem" }}>{feature}</span>
                    </li>
                  ))}
                </ul>
      ...
      <section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        <h2 className="text-center mb-10" style={{ fontSize: "1.75rem", fontWeight: 700, color: "#1A1818" }}>
          Câu Hỏi Về Bảng Giá
        </h2>
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            ...
            >
              <h4 className="mb-2 font-semibold" style={{ color: "#1A1818" }}>{faq.q}</h4>
              <p style={{ color: "#6B6B6B", lineHeight: 1.6, fontSize: "0.9375rem" }}>{faq.a}</p>
            </motion.div>
          ))}
        </div>
        <div className="text-center mt-8">
          <a href="/faq" style={{ color: "#E8B4A8", fontWeight: 600, fontSize: "0.9375rem" }}>
