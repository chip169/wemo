...
export function AIChibiPage() {
  ...
  const [isVip, setIsVip] = useState(() => {
    try {
      return localStorage.getItem("wemo_chibi_vip") === "true";
    } catch {
      return false;
    }
  });
  ...
  const handleActivateCode = () => {
    if (inputCode.trim() === "1901") {
      try {
        const currentCount = getUsageCount();
        const newCount = Math.max(0, currentCount - 3);
        localStorage.setItem("wemo_chibi_count", String(newCount));
        localStorage.setItem("wemo_chibi_vip", "true");
        setUsageCount(newCount);
        setIsVip(true);
        setActivationMessage({ text: "Kích hoạt thành công! Nhận thêm 3 lượt tạo.", type: "success" });
        setInputCode("");
      } catch (err) {
        setActivationMessage({ text: "Không thể kích hoạt mã.", type: "error" });
      }
    } else {
      setActivationMessage({ text: "Mã kích hoạt không chính xác.", type: "error" });
    }
  };
          ...
          <div className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-2xl border font-bold text-xs shadow-sm ${
            ...
            {isLimitReached ? (
              ...
              <>
                ...
                {isVip && <span className="text-[10px] font-black uppercase bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-full mr-1.5">VIP</span>}
                    ...
                    {showCamera ? (
                      ...
                      <div className="space-y-4">
                        <div
                          ...
                          className={`border-2 border-dashed rounded-3xl p-8 flex flex-col items-center justify-center transition-all ${isLimitReached
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-3xl p-5 border border-stone-200/60 shadow-lg text-left space-y-5">
                  ...
                  {CHIBI_STYLES.map((style) => {
                    ...
                    return (
                      <motion.button
                        ...
                        disabled={isLimitReached}
                        className={`p-3.5 rounded-2xl border text-left flex items-start gap-3 transition-all relative overflow-hidden cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${isSelected
              ...
              <div className="pt-3 border-t border-stone-100 space-y-3">
                <div className="space-y-2 bg-[#E8B4A8]/5 border border-[#E8B4A8]/20 p-4 rounded-2xl text-left">
                  <label className="block text-[10px] font-black text-[#5C4839] uppercase tracking-wider">
                    🎟️ Nhập Mã Kích Hoạt VIP
                  </label>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded-xl border border-stone-200 outline-none focus:border-[#E8B4A8] text-xs bg-white text-stone-700 font-medium transition-colors"
                    />
                    <button
                      type="button"
                      onClick={handleActivateCode}
                      className="px-4 py-2 rounded-xl bg-gradient-to-r from-[#E8B4A8] to-[#D4AF78] text-white text-xs font-black shadow hover:opacity-95 transition-all cursor-pointer whitespace-nowrap"
                    >
                      Kích hoạt
                    </button>
                  </div>
                  {activationMessage && (
                    <p className={`text-[10px] font-bold ${activationMessage.type === "success" ? "text-emerald-600" : "text-rose-600"}`}>
            ...
            <div className="bg-white rounded-3xl p-5 border border-stone-200/60 shadow-sm text-left space-y-3">
              ...
              {[
                ...
                <div key={item.step} className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${item.done ? "bg-[#E8B4A8] text-white" : "bg-stone-100 text-stone-400"
                    }`}>
